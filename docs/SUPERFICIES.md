...
| OSD del video | ✅ | R2-F4/M-060/M-061 | `mpegplayer.c` rediseñado con estilo Zune HD real: fila única ícono geométrico en acento + tiempos en los extremos de la barra + línea de 2px con punta redondeada (pista al 28% blanco) + barra de nivel de volumen (sin "dB", pedido del dueño), tipografía `metro-caption-14.fnt`, panel plano sin bisel. **Verificado visualmente**: `docs/screenshots/R2-F4-zune-osd.png` -- la imposibilidad que reportaban R2-3/M-059/M-060 era un binario desactualizado en el simdisk (`make` no instala plugins, `make install` sí; ver `docs/DESVIACIONES.md` R2-5). El panel sigue siendo opaco por arquitectura (recorte de clip-rect, no compositing -- `docs/DESVIACIONES.md` R2-4) |
...
| Menús Settings/Display/Audio de mpegplayer | ✅ | R2-F4/M-061 | Reemplazados por `metro_menu_draw()`/`metro_menu_pick()` con la anatomía completa de página Metro (M-061): ceja+reloj+batería, título gigante minúsculas en display-48, fuentes list/listsel reales, valores que ciclan en el lugar con subtítulo visible (patrón de `metro_screen_settings.c`, no selectores). Verificado: `docs/screenshots/R2-F4-zune-menu-video.png`, `R2-F4-zune-menu-ajustes.png`, `R2-F4-zune-menu-pantalla.png`, `R2-F4-zune-menu-audio.png`, `R2-F4-zune-brillo.png` (las capturas previas de M-059 quedan como histórico del estado intermedio) |
